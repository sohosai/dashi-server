...
pub(super) async fn rent(
    ...
    if rental_data.recipient.chars().count() == 0 {
        return Err(RentRentalError::RecipientEmptyError);
    }
    ...
    let item_model = match Item::find_by_id(rental_data.id).all(&rdb).await {
        Ok(item_models) => {
            if item_models.len() > 1 {
                ...
                return Err(RentRentalError::IdConflictInItemTableError);
            }
            if item_models.is_empty() {
                // If id does not exist
                return Err(RentRentalError::IdNotFoundInItemTableError);
            }
            item_models[0].clone()
        }
        Err(e) => return Err(RentRentalError::RDBError(e)),
    };
    ...
    if meilisearch_item.len() > 1 {
        ...
        //* critical incident *//
        critical_incident::conflict_error().await;
        return Err(RentRentalError::IdConflictInMeiliSearchError);
    }
    if meilisearch_item.is_empty() {
        // If visible_id does not exist
        return Err(RentRentalError::IdNotFoundInMeiliSearchError);
    }
    ...
    if item_model.is_rent {
        // If is_rent is true
        return Err(RentRentalError::AlreadyRentedInItemTableError);
    }
    // validation of is_rent is false in MeiliSearch
    if meilisearch_item[0].is_rent {
        // If is_rent is true
        return Err(RentRentalError::AlreadyRentedInMeiliSearchError);
    }
    ...
    if rental_data.scheduled_replace_at.chars().count() > 0 {
        ...
        scheduled_replace_at = Some(
            DateTime::parse_from_rfc3339(&rental_data.scheduled_replace_at)
                .map_err(|e| RentRentalError::ParseDateTimeError(e.to_string()))?,
        );
    }
    let latest_rent_at = Some(chrono::Utc::now().naive_utc());
    //* update MeiliSearch *//
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            return Err(RentRentalError::MeiliSearchError(e));
        }
    }
    ...
    active_item_model.is_rent = Set(true);
    ...
    active_item_model.latest_rent_at = Set(latest_rent_at);
    active_item_model.latest_replace_at = Set(None);
